...
- 特殊注释前缀。FIXME(有待修复), TODO(未完成代码), FIXME(修复我), HACK(比较 trick 才能实现的逻辑说明), NOTE(代码注意事项)
...
- 依赖 bug。一般经过广泛使用的第三方库是可以信赖的，但是公司自己造的轮子（尤其是文档和单测都没有的），还是有可能出 bug 的。有可能是依赖而非自己代码逻辑 bug。
...
- 配置/环境问题。是否是因为配置而非代码逻辑 bug 导致的，线上/测试/开发环境 的配置是否正确，是否写串了。
